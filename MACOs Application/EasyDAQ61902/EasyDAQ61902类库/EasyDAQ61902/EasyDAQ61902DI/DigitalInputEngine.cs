...
{
    /// <summary>
    /// Reflection engine for DI
    /// </summary>
    public class DigitalInputEngine
    {
        ...
        public bool Initialize(int boardID)
        {
            try
            {
                ...
                pvType = assem.GetType(driverClassName + "." + driverClassName + "DITask");
                // Instance 
        ...
        public bool[] ReadData()
        {
            try
            {
                Type[] readSignlePointType = new Type[] { typeof(bool[]).MakeByRefType() };
                pvParam = new object[] {data};
                MethodInfo readdata = device.GetType().GetMethod("ReadSinglePoint", readSignlePointType);
                readdata.Invoke(device, pvParam);
                return data;
            }
            catch (Exception ex)
            {
