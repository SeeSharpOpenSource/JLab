...
namespace FrequencyResponseFunctionDemo
{
    public partial class Form1 : Form
    {
        ...
        SpeakAudioTest speakTest;
        SpeakAudioTest.WaveType GenWaveType;
        ...
        private void buttonRun_Click(object sender, EventArgs e)
        {
            ...
            if (comboBoxSignal.Text == "WhiteNoise")
            {
                ...
                else
                {
                    ...
                    //将Input数据写入AO0并从AI0读取出来
                    AOWriteAIRead();

                    //将测试数据保存为.bin文件
                    WriteData("WhiteNoise");
                }
                WhiteNoiseAnalysis();
                easyChartTHD.Clear();
            }

            if (comboBoxSignal.Text == "Chirp")
            {
                speakTest = new SpeakAudioTest();
                ...
                {          
                    //将Input数据写入AO0并从AI0读取出来
                    writeValue = speakTest.TimeWaveform;
                    AOWriteAIRead();
                    speakTest.TimeWaveform = TempInput;
                    //将测试数据保存为.bin文件
                    WriteData("Chirp");
                }
                ChirpAnalysis();
        ...
        /// SpeakTest初始化配置
        /// </summary>
        private void SpeakTestInitial()
        {
            switch (comboBoxWaveType.SelectedText)
            {
                case "Sine":
                    GenWaveType = SpeakAudioTest.WaveType.Sine;
                    break;
                case "Square":
                    GenWaveType = SpeakAudioTest.WaveType.Square;
                    break;
                case "Saw":
                    GenWaveType = SpeakAudioTest.WaveType.Saw;
                    break;
                default:
                    GenWaveType = SpeakAudioTest.WaveType.Sine;
                    break;
            }
            speakTest.AnalysisFStart = (double)numericUpDownChirpFrequencyStart.Value;
            speakTest.AnalysisFStop = (double)numericUpDownChirpFrequencyStop.Value;
            speakTest.SampleRate = (double)numericUpDownSamplingRate.Value;
            speakTest.HighestHarmonic = (int)numericUpDownHighestHamonic.Value;
            speakTest.InitChirpParameters((int)numericUpDownTestTime.Value, GenWaveType);
        }
        /// <summary>
        /// 分析StepSine数据
        /// </summary>
        ...
        /// 分析Chirp数据
        /// </summary>
        private void ChirpAnalysis()
        {
            speakTest.ResponseWaveform = TempOutput;
            speakTest.AnalysisChirp();
            easyChartInput.Plot(TempInput);
            easyChartOutput.Plot(TempOutput);
            double[] LevelOfFrequenciesInDB = new double[speakTest.LevelOfFrequencies.Length];
            for(int i = 0; i < LevelOfFrequenciesInDB.Length; i++)
            {
                LevelOfFrequenciesInDB[i] = 20 * Math.Log10(speakTest.LevelOfFrequencies[i]);
            }
            easyChartMagnitude.Plot(speakTest.Frequencies, LevelOfFrequenciesInDB);
            easyChartTHD.Plot(speakTest.Frequencies, speakTest.THDofFrequencies);
        }
        /// <summary>
        /// 读取StepSine数据
        /// </summary>
        ...
        {
            string FilePath = Environment.CurrentDirectory.Replace("SpeakerTestDemo\\bin\\Debug", "Source\\"+name+".bin");
            FileStream fs = new FileStream(FilePath, FileMode.Open);
            BinaryReader fileReadHandle = new BinaryReader(fs);
            long fileLength = fs.Length;
            byte[] bytebuffer = new byte[fileLength / 2];
        ...
        {
            string FilePath = Environment.CurrentDirectory + @"\" + name + @".bin";
