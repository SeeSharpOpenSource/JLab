...
{
    class JYPXI69527 : StreamingTask
    {
        #region Private filed
        private readonly Type streamRawDataType = typeof(int);
        private JYPXI69527AITask aitask;
        ...
        /// 流盘预览缓冲内存
        /// </summary>
        private JYCommon.CircularBuffer<int> _localPreviewBuffer;
        /// <summary>
        /// 写二进制对象
        /// </summary>
        ...
        /// 流盘预览缓冲转换前的数组
        /// </summary>
        private int[,] _previewDataConvertBuffer;
        /// <summary>
        /// 流盘是否完成
        /// </summary>
        ...
        public override double GetVendorRange(double rangeLow, double rangeHigh)
        {
            ...
            if (rangeLow >= -0.316 && rangeHigh <= 0.316)
            {
                return 0.316;
            }
            ...
            else if (rangeLow >= -3.16 && rangeHigh <= 3.16)
            {
                return 3.16;
            }
            else if (rangeLow >= -10 && rangeHigh <= 10)
            {
                return 10.0;
            }
            else
        ...
        private void ScaleRawData(int[,] rawbuf, ref double[,] scaledData, int samples, double scaleRange)
        {
            for (int i = 0; i < samples; i++)
            {
                for (int j = 0; j < rawbuf.GetLength(1); j++)
                {
                    scaledData[i, j] = (double)(rawbuf[i, j] * 256) * scaleRange / int.MaxValue;
                }
            }
        }
        ...
        {
            _recordBlockSize = (int)GetNearest2N((uint)(aitask.SampleRate / 5)) * aitask.Channels.Count;
