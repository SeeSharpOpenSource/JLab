...
{
    class JYPXI62204 : StreamingTask
    {
        #region Private filed
        private readonly Type streamRawDataType = typeof(short);
        private JYPXI62204AITask aitask;
        ...
        public JYPXI62204(int boardNum)
        {
            aitask = new JYPXI62204AITask(boardNum);
        ...
        public override void Record(int chnID, double sampleRate, double recordLength, double rangelow = -10, double rangeHigh = 10)
        {
            ...
            aitask.AddChannel(chnID, rangelow, rangeHigh, (Coupling)Enum.Parse(typeof(Coupling), Coupling.ToString()),
                (AITerminal)Enum.Parse(typeof(AITerminal), TerminalCfg.ToString()));
        ...
        {
            if (rangeLow >= -0.05 && rangeHigh <= 0.05)
            {
                return 0.05;
            }
            ...
            else if (rangeLow >= 0.25 && rangeHigh <= 0.25)
            {
                return 0.25;
            }
            else if (rangeLow >= -0.5 && rangeHigh <= 0.5)
            {
                return 0.5;
            }
            else if (rangeLow >= -1.0 && rangeHigh <= 1.0)
            {
                return 1.0;
            }
            else if (rangeLow >= -1.25 && rangeHigh <= 1.25)
            {
                return 1.25;
            }
            else if (rangeLow >= -2.0 && rangeHigh <= 2.0)
            {
                return 2.0;
            }
            else if (rangeLow >= -2.5 && rangeHigh <= 2.5)
            {
                return 2.5;
            }
            else if (rangeLow >= -5.0 && rangeHigh <= 5.0)
            {
                return 5.0;
            }
            else
